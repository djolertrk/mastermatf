...
\apstr{%
  Potreba za razvojom softvera raste svakodnevno. Softver je svuda oko nas. Da bi se podigla efikasnost programera dolazi do potrebe za raznim alatima koji služe za pronalaženje i otklanjanje grešaka. Jedan takav alat je debager \emph{GNU GDB}.
  
  Višenitni softver je veoma prisutan, ponekad i neizbežan u softveru na uređajima sa ugrađenim računarom. Često takvi uređaji nemaju debager, pa je analiza programa moguća samo na ličnim računarima. Jedan način analize grešaka u programima sa takvih uređaja je učitavanje datoteke jezgra u debager.
  
  Koriščenje \emph{GNU GDB} alata na arhitekturi domaćina prilikom analize datoteka jezgara koje su generisane na ugrađenim uređajima drugih arhitektura je bilo uskraćeno za čitanje vrednosti \emph{TLS} (eng. \emph{Thread Local Storage}) promenljivih. Ovaj rad opisuje unapređenje alata radi dodavanja tog ponašanja.
}
